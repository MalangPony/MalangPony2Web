...
for (const tier_id in tiers_data){
	let tier=tiers_data[tier_id];
	...
	while (tier_ptr){
		...
		perks_id_list=tiers_data[tier_ptr].perks_list.concat(perks_id_list);
		// Set the original-tier map
		for (const perk_id of tiers_data[tier_ptr].perks_list){
			perks_original_tier[perk_id]=tier_ptr;
		}
		// Walk up the inheritance chain
		tier_ptr=tiers_data[tier_ptr].inherits;
	}
	...
	for( const perk_id of perks_id_list ){
		...
		perk_dom.querySelector(".perk-name .langspan-ko").innerHTML=
			perks_data[perk_id].name_ko;
		perk_dom.querySelector(".perk-name .langspan-en").innerHTML=
			perks_data[perk_id].name_en;
		
		let desc_ko=perks_data[perk_id].desc_ko;
		...
		perk_dom.querySelector(".register-tier-perk").classList.add(
			tiers_data[perks_original_tier[perk_id]].css_class);
